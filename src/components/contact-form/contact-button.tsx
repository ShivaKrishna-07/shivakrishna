...
export default function ContactButton() {
  ...
  return (
    <>
      ...
      <button
        ref={refSendBtn}
        className="inline-flex items-center rounded-md bg-background px-3 py-2 text-accent transition-transform ml-2 duration-150 focus-within:scale-[1.05] hover:scale-[1.05] hover:bg-foreground hover:text-background"
        onClick={() => setIsOpenModal(true)}
      >
        <HiMail className="h-6 w-6 sm:h-7 sm:w-7 lg:h-6 lg:w-9" />
        <span className="text-base font-semibold sm:text-lg lg:text-lg">
          Send Message
        </span>
      </button>
